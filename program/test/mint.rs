...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("avn_nft", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let aircraft_type: String = Default::default();
		let flight_route: String = Default::default();
		...
		let (avn_pubkey, _) = Pubkey::find_program_address(
			&[b"avn", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(avn_pubkey, false),
		...
		let data = AvnNftInstruction::Mint(crate::generated::instructions::MintArgs{
			aircraft_type,
			flight_route,
			description,
		});
